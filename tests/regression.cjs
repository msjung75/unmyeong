...
(async () => {
  ...
  assert.equal(w.document.querySelectorAll('.card-pillars .tile').length,12);
  ...
  assert.equal(w.document.querySelectorAll('.card-pillars .pcol').length,6);
  assert.equal(w.document.querySelectorAll('.card-pillars .pcol .tile.empty').length,6);
  ...
  await run('openChatGptReading(currentId,2027)');
  assert.equal(opened.length,1,'복사 실패 시 외부 탭을 열지 않는다');
